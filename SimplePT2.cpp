...
struct SpecularBRDF: public BRDF {
	...
	Vec eval(const Vec &n, const Vec &o, const Vec &i) const {
		Vec temp = n * 2.0 * n.dot(o) - o;
		if (i.x == temp.x and i.y == temp.y and i.z == temp.z){
			return ks * (1/n.dot(i));
		}
		else{
...
Vec receivedRadiance(const Ray &r, int depth, bool flag) {
    ...
    if (obj.brdf.isSpecular())
	{
		...
		if (rng() < p)
		{
			Vec i = uniRandomPSA(n);
			brdf.sample(n,o,i,pdf);
			Ray y(x, i);
			Le = Le + receivedRadiance(y, depth+1, flag).mult(brdf.eval(n,o,i)) * (n.dot(i) / (pdf*p));
		}
		return Le;
	}
	else
	{
		Le = Le + reflectedRadiance(r, depth);
		return Le;
	}
}
